export class FileOrganizerSettings {
  ...
  // 可选：对三级目录的含义做提示，便于后续二次 AI 选择
  level3DirHints: Record<string, string> = {
    "01.Roadmap": "路径规划, 知识点地图, 索引归档笔记双链",
    "02.What": "核心概念",
    "03.Why": "准则原则思想等，'道法术'里的'道'",
    "04.How": "标准操作流程、最佳实践、教程等，'道法术'里的'法'",
    "05.Tool": "具体的工具，'道法术'里的'术'",
    "06.Resource": "领域相关资源、素材库",
  };

  // 目录创建策略
  // true: 选定二级后，立即创建所有标准三级目录（当前实现）
  // false: 仅在最终确定的三级目录落位前，懒创建该目录（推荐）
  eagerCreateLevel3Dirs = false;
  // 将已存在的自定义三级目录（非标准清单）一并纳入二次 AI 候选
  includeExistingLevel3Dirs = true;
  // 二次 AI 无法判定时，默认落位的三级目录名（需存在于 level3Dirs 中）
  fallbackLevel3Dir = "02.What";
  // Roadmap 生成提示词（可自定义），默认使用 docs/flow/参考流程.md 中的模板
  roadmapPrompt = `我想系统性地学习【领域名称】，您是这个领域中拥有超过20年经验的顶尖专家和资深教育家。你不仅精通该领域的全部理论知识和前沿动态，更擅长将复杂庞大的知识体系，为不同阶段的学习者设计出清晰、高效、循序渐进的学习路径。请为我设计一个完整的学习路线图。\n\n## 任务要求：\n1. 将学习内容分为初级、中级、高级三个层次\n2. 每个层次列出3-5个核心知识模块\n3. 每个模块包含3-5个关键知识点\n4. 提供学习时间估算和前置要求\n5. 推荐1-2个该层次的优质学习资源\n\n## 输出格式：\n\n### 🎯 领域概览\n- **简介**: 该领域的定义和特点\n- **应用场景**: 主要应用场景和发展趋势\n- **学习目标**：[简述掌握该领域后能达到的能力水平]\n- **总学习周期**：[预估完整学习所需时间]\n\n### 📚 初级（入门基础）\n**学习目标**：[该阶段要达到的具体能力]\n**预计时间**：[学习时间估算]\n**前置要求**：[需要具备的基础知识]\n\n#### 1.[模块名称]\n- **知识点1：[具体知识点]**\n\n- **知识点2：[具体知识点]**\n\n- **知识点3：[具体知识点]**\n\n#### 2.[模块名称]\n- **知识点1：[具体知识点]**\n\n- **知识点2：[具体知识点]**\n\n[继续其他模块...]\n\n#### 推荐资源：\n- [资源1名称]：[简要说明]\n- [资源2名称]：[简要说明]\n\n### 🚀 中级（进阶提升）\n[按相同格式展开]\n\n### 🎓 高级（专业精通）\n[按相同格式展开]\n\n### 💡 学习建议\n1. **学习顺序**：[建议的学习路径]\n2. **实践项目**：[每个阶段建议的实践项目]\n3. **评估标准**：[如何判断是否掌握该阶段内容]\n4. **常见误区**：[学习过程中需要避免的问题]\n\n### 输出要求：\n1. 内容要具体详实，避免空泛的建议\n2. 提供可操作的学习步骤和方法\n3. 包含具体的资源推荐和工具建议\n\n现在请根据给定领域生成具体的学习路线图。`;

  // 步骤3: 内容优化和格式化提示词（可自定义）
  // 用于当基于模板的格式化指令缺失时，或作为统一的优化/格式化指令传递给格式化接口
  optimizePrompt = `请将下面的笔记内容优化为高质量的 Markdown 教学/参考笔记：\n\n要求：\n1. 保持语义完整，提取并放大核心要点；\n2. 统一并优化标题、层级结构（使用合理的 H1-H3）；\n3. 提取摘要（100 字内），并在开头插入；\n4. 修复明显的语法和事实性错误；\n5. 如内容为非中文，请翻译为地道中文；\n6. 保留并正确格式化代码块、表格、引用和列表；\n7. 对长段落进行适当分段，并在必要处添加小结或步骤列表；\n8. 输出为纯 Markdown 文本，不添加多余的注释或说明。\n\n占位符说明：$\{filename\} - 文件名，$\{content\} - 原始内容\n\n原始内容：\n$\{content\}`;
}
